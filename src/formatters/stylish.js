    ...
    const strings = subTree.map((node) => {
      switch (node.type) {
        ...
        case 'removed':
          return `${indent}- ${node.key}: ${_.isObject(node.value) ? iter(Object.entries(node.value), depth + 2) : node.value}`;
        case 'changed':
          return `${indent}- ${node.key}: ${_.isObject(node.value.value1) ? iter(Object.entries(node.value.value1), depth + 2) : node.value.value1}\n${indent}+ ${node.key}: ${_.isObject(node.value.value2) ? iter(Object.entries(node.value.value2), depth + 2) : node.value.value2}`;
        case 'unchanged':
          return `${indent}  ${node.key}: ${_.isObject(node.value) ? iter(Object.entries(node.value), depth + 2) : node.value}`;
        case 'nested':
          return `${indent}  ${node.key}: ${iter(node.value, depth + 2)}`;
        default:
